...
def _is_numeric(s) -> bool:
    """Version-robust numeric check (works across pandas 1.x–3.x)."""
    return pd.api.types.is_numeric_dtype(s) and not pd.api.types.is_bool_dtype(s)


def _is_stringlike(s) -> bool:
    """
    Version-robust string/object check.
    Pandas 3.0 introduced 'str' dtype; earlier versions use 'object'.
    """
    if pd.api.types.is_object_dtype(s):
        return True
    # pandas >= 2.1 StringDtype, pandas 3.0 'str'
    try:
...
def _name_looks_like_id(col_name: str) -> bool:
    ...
    return any(tok in ID_NAME_HINTS for tok in tokens)
...
class DataProfiler:
    ...
    def _infer_type(self, s, dtype_str: str, uniq: int,
        ...
        if _is_stringlike(s) and self._looks_like_datetime(s):
            return ColType.TEMPORAL, {"parsed": False, "needs_parse": True}, []

        # ── 2. Year column heuristic ──────────────────────────────────────
        if re.search(r"\byear\b", col_name, re.I) and _is_numeric(s):
            s_clean = s.dropna()
            if len(s_clean) and (1900 < float(s_clean.min()) < 2100):
                return ColType.TEMPORAL, {"part": "year"}, ["Year-only column — no sub-year granularity"]

        # ── 3. Name-based identifier detection (BEFORE numeric measure) ────
        # Columns literally named like IDs are never measures, even if numeric.
        if _name_looks_like_id(col_name) and (uniq > self.cutoff or ratio >= ID_RATIO_THRESHOLD):
            return ColType.IDENTIFIER, {"reason": "name+uniqueness"}, []

        # ── 4. Numeric columns (measures / coded categoricals) ────────────
        # IMPORTANT: check numeric BEFORE the generic high-uniqueness identifier
        # rule, so a continuous measure like `revenue` (ratio 0.95) is not
        # mistaken for an ID.
        if _is_numeric(s):
            # Low-cardinality integers → coded categorical dimension
        ...
        # ── 5. Boolean (exactly 2 distinct, non-numeric) ──────────────────
        if uniq == 2:
            return ColType.BOOLEAN, {}, []

        # ── 6. String / object columns ────────────────────────────────────
        if _is_stringlike(s):
            non_null = s.dropna().astype(str)
            avg_len = float(non_null.str.len().mean()) if len(non_null) else 0.0
            # Long free-text → exclude
            if avg_len > TEXT_AVG_LEN_THRESHOLD:
                return ColType.TEXT, {"avg_len": round(avg_len, 1)}, []
            # High-uniqueness string with id-ish name already caught in step 3.
            # Otherwise: string identifier only if nearly unique.
            if ratio >= ID_RATIO_THRESHOLD and uniq > self.cutoff:
                return ColType.IDENTIFIER, {"ratio": round(ratio, 3)}, []
            # Low-cardinality → cube dimension
            if uniq <= self.cutoff:
                return ColType.DIMENSION, {"cats": uniq}, []
            # Above cutoff but still categorical → excluded (SQL-only) dimension
    ...
    def _looks_like_datetime(self, s: pd.Series) -> bool:
        ...
        for fmt in self.DATETIME_FORMATS:
            try:
                ...
                pass
        # Last resort: let pandas infer, but suppress the noisy fallback warning
        try:
            import warnings
